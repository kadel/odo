  ...
  odo create python`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(componentGit) != 0 {
			...
			checkError(err, "")
			err = component.CreateFromDir(client, componentName, componentType, dir, applicationName, "local")
			checkError(err, "")
			...
			err = component.CreateFromDir(client, componentName, componentType, path, applicationName, "binary")
			checkError(err, "")
			fmt.Printf("Please wait, creating %s component ...\n", componentName)
			err = component.Build(client, componentName, false, true)
			checkError(err, "")
